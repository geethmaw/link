...
# @Last modified time: 2022-06-06T16:39:57-06:00



### compare gcms, observations U10 vs M oceans 30N to 70N.
...
### GCM
modname = ['CESM2','CESM2-WACCM','HadGEM3-GC31-LL','HadGEM3-GC31-MM','IPSL-CM6A-LR','CNRM-ESM2-1','INM-CM5-0','MPI-ESM1-2-HR','UKESM1-0-LL']
...
### CAOI at 800hPa
    ...
    # fig = plt.figure()
    #plt.plot(np.sort(yy[indx]), norm.pdf(np.sort(yy[indx])), label=modname[i], alpha=0.6)
    # plt.legend()
    ...
    # from scipy.stats import norm
    # fig = plt.figure()
    plt.plot(xx[indx], norm.pdf(xx[indx]), label=modname[i], alpha=0.6)
    # # plt.legend()
...
#plt.plot(test_yy_new, norm.pdf(test_yy_new), color='black',linestyle='dashed', label='observations')
##############################################
...
plt.plot(test_xx_new, norm.pdf(test_xx_new), color='black',linestyle='dashed', label='observations')
##############################################

plt.legend()
plt.title('U10 PDF')
plt.savefig('../figures/new_nineGCMs_MPDF.png')
